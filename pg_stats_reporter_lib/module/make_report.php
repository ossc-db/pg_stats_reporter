...
function makeReport($conn, $config, $url_param, &$err_msg)
{
	...
	if ($config[$url_param['repodb']]['repo_version'] < V11) {
		$err_msg = sprintf($error_message['st_version'], "11.x");
		return null;
	}
...
function makeReportForCommandline($conn, $infoData, $target_info, $snapids)
{
	...
	/* check repository version */
	if ($infoData[$target_info['repodb']]['repo_version'] < V11) {
		return null;
	}
