...
$(function(){
	...
	$("#accordion a").each(function(){
		var php_file = window.location.pathname.split('/').pop();
		$(this).attr('href', php_file + "?" + $(this).attr('href'));
	});
		...
		buttons: {
			"Create report": function(){
				...
				if (date_b == ''||date_e == '') {
					...
					// URLパラメータの生成
					if (!(("repodb" in url_param) && ("instid" in url_param))) {
						// アコーディオンの先頭の情報を取得する
						...
						var harg = new Object;
						for (var i = 0; hparam[i]; i++ ) {
							var hpair = hparam[i].split('=');
							harg[hpair[0]] = hpair[1];
						}
						url_param["repodb"] = harg["repodb"];
						url_param["instid"] = harg["instid"];
					}
					url_param["begin"] = document.getElementById("begin_date").value;
					url_param["end"] = document.getElementById("end_date").value;

					document.location = php_file + "?" + $.fn.createUrlString(url_param);
				}
			},
			Cancel: function(){
				$(this).dialog("close");
	...
	.click(function(){
		...
		if (url_param) {
			...
			if (url_param.search("reload") > 0) {
				...
				document.location = php_file + location.search;
			} else {
				// reloadのパラメータがついていない場合
				document.location = php_file + location.search + "&reload=1";
			}
		} else {
			// URLパラメータを持っていない場合
			document.location = php_file + "?reload=1";
		}
	});
